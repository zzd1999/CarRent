...
<head>
    ...
    <link rel="stylesheet" href="http://localhost:8080/CarRent_war_exploded/resources/css/public.css" media="all"/>
</head>
...
<script src="http://localhost:8080/CarRent_war_exploded/resources/layui/layui.js"></script>
<script type="text/javascript">
    ...
    layui.use(['jquery', 'layer', 'form', 'table', 'upload'], function () {
        ...
        tableIns = table.render({
            ...
            , url: 'http://localhost:8080/CarRent_war_exploded/car/loadAllCar.action' //数据接口
            ...
            , cols: [[   //列表数据
                ...
                , {
                    field: 'carimg', title: '缩略图', align: 'center', width: '80', templet: function (d) {
                        return "<img width=40 height=40 src=http://localhost:8080/CarRent_war_exploded/file/downloadShowFile.action?path=" + d.carimg + "/>";
                    }
                }
...
//            alert(params);
            tableIns.reload({
                url: "http://localhost:8080/CarRent_war_exploded/car/loadAllCar.action?" + params,
                page: {curr: 1}
            })
        });
        ...
        table.on('tool(carTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('真的删除【' + data.carnumber + '】这个车辆么？', function (index) {
                    //向服务端发送删除指令
                    $.post("http://localhost:8080/CarRent_war_exploded/car/deleteCar.action", {carnumber: data.carnumber}, function (res) {
                        layer.msg(res.msg);
                        //刷新数据表格
                        tableIns.reload();
                    })
        ...
        function openAddCar() {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#dataFrm")[0].reset();
                    //设置默认图片
                    $("#showCarImg").attr("src", "http://localhost:8080/CarRent_war_exploded/file/downloadShowFile.action?path=images/defaultcarimage.jpg");
                    $("#carimg").val("images/defaultcarimage.jpg");
                    url = "http://localhost:8080/CarRent_war_exploded/car/addCar.action";
        ...
        function openUpdateCar(data) {
            mainIndex = layer.open({
                ...
                success: function (index) {
                    ...
                    $("#showCarImg").attr("src", "http://localhost:8080/CarRent_war_exploded/file/downloadShowFile.action?path=" + data.carimg);
                    url = "http://localhost:8080/CarRent_war_exploded/car/updateCar.action";
        ...
        function deleteBatch() {
            ...
            layer.confirm('真的要删除这些车辆么？', function (index) {
                //向服务端发送删除指令
                $.post("http://localhost:8080/CarRent_war_exploded/car/deleteBatchCar.action", params, function (res) {
                    layer.msg(res.msg);
                    //刷新数据表格
                    tableIns.reload();
                })
        ...
        upload.render({
            elem: '#carimgDiv',
            url: 'http://localhost:8080/CarRent_war_exploded/file/uploadFile.action',
            method: "post",  //此处是为了演示之用，实际使用中请将此删除，默认用post方式提交
            acceptMime: 'images/*',
            field: "mf",
            done: function (res, index, upload) {
                $('#showCarImg').attr('src', "http://localhost:8080/CarRent_war_exploded/file/downloadShowFile.action?path=" + res.data.src);
        ...
        function showCarImage(data) {
            mainIndex = layer.open({
                ...
                area: ['750px', '500px'],
                success: function (index) {
                    $("#view_carimg").attr("src","http://localhost:8080/CarRent_war_exploded/file/downloadShowFile.action?path="+data.carimg);
                }
            });
        }
