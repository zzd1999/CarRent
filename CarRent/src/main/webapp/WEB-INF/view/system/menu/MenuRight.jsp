    <%@ page language="java" contentType="text/html; charset=UTF-8"
        ...
        ,url:'http://localhost:8080/CarRent_war_exploded/menu/loadAllMenu.action' //数据接口
        ...
        alert(params);
        tableIns.reload({
        url:"http://localhost:8080/CarRent_war_exploded/menu/loadAllMenu.action?"+params
        })
        });
        ...
        if(layEvent === 'del'){ //删除
        //先判断当前菜单有没有子节点
        $.post("http://localhost:8080/CarRent_war_exploded/menu/checkMenuHasChildren.action?id="+data.id,function(obj){
        if(obj.code>=0){
                layer.msg("当前菜单有子节点，请先删除完子节点！");
        }else{
        layer.confirm('确定删除【'+data.title+'】这个菜单吗？', function(index){
        //向服务端发送删除指令
        $.post("http://localhost:8080/CarRent_war_exploded/menu/deleteMenu.action",{id:data.id},function(res){
        layer.msg(res.msg);
        //刷新数据 表格
        tableIns.reload();
        //刷新左边的树
        ...
        url="http://localhost:8080/CarRent_war_exploded/menu/addMenu.action";
        }
        });
        }
        //打开修改页面
        ...
        //移除打开的样式
       $("#menuSelectDiv").removeClass("layui-show");
        url="http://localhost:8080/CarRent_war_exploded/menu/updateMenu.action";
        //反选下拉树
        var pid=data.pid;
        ...
        url: "http://localhost:8080/CarRent_war_exploded/menu/loadMenuManagerLeftTreeJson.action?spread=1",  // 使用url加载（可与data加载同时存在）
        icon: "2",
        accordion:true
        });
        ...
        tableIns.reload({
        url:"http://localhost:8080/CarRent_war_exploded/menu/loadAllMenu.action?id="+id
        })
        }
        </script>
        </body>
